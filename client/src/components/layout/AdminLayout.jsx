...
export const AdminLayout = () => {
  ...
  const menuItems = [
    ...
    { name: 'KYC Verification', path: '/admin/kyc', icon: ShieldAlert },
    { name: 'Users', path: '/admin/users', icon: Users },
    { name: 'Support Tickets', path: '/admin/tickets', icon: LifeBuoy },
    { name: 'Settings & Audit', path: '/admin/settings', icon: Settings }
  ];
  ...
  return (
    <div className="min-h-screen bg-slate-100 flex flex-col md:flex-row">
      {/* Desktop Sidebar */}
      <aside className="hidden md:flex flex-col w-64 bg-slate-900 text-slate-200 shrink-0 sticky top-0 h-screen overflow-y-auto">
        ...
        <div className="px-5 py-4 border-b border-slate-800 bg-slate-800/40">
          ...
          <span className="inline-block mt-1 px-2 py-0.5 rounded bg-indigo-900/60 text-indigo-300 text-[10px] font-bold border border-indigo-700/50">
            {user?.role}
          </span>
        </div>
      ...
      {/* Mobile Top Header */}
      <div className="md:hidden sticky top-0 z-30 bg-slate-900 text-white px-4 py-3 flex items-center justify-between border-b border-slate-800">
        <div className="flex items-center gap-2.5">
          ...
          <span className="font-extrabold text-white tracking-tight text-base">
            SIMPL<span className="text-indigo-400">DSC</span> Admin
          </span>
        </div>
      ...
      {mobileDrawerOpen && (
        <div className="fixed inset-0 z-50 flex md:hidden">
          ...
          />
          <div className="relative w-4/5 max-w-xs bg-slate-900 text-white h-full shadow-2xl flex flex-col z-10">
            <div className="p-4 border-b border-slate-800 flex items-center justify-between">
              <span className="font-bold text-white">Admin Management</span>
              <button
                onClick={() => setMobileDrawerOpen(false)}
